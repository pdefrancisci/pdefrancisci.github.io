...
import com.webcheckers.ui.Lobby.*;
...
public class WebServer {
    ...
    public void initialize() {
        ...
        // Handles logging out
        get(SIGNOUT_URL, new GetSignoutRoute(templateEngine, gameCenter));

        // Handles game
        get(GAME_URL, new GetGameRoute(templateEngine, gameCenter));

        //handles spectate
        get(SPECTATE_URL, new GetSpectateRoute(templateEngine,gameCenter));
        post( "/spectator/checkTurn", new PostSpectateCheckTurn(templateEngine, gameCenter));

        post(CHECKTURN_URL, new PostCheckTurn(templateEngine, gameCenter));

        post(VALIDATE_URL, new PostValidateMove(templateEngine, gameCenter));

        post(SUBMIT_TURN, new PostSubmitTurn(gameCenter));

        post(RESIGN_TURN, new PostResign(templateEngine, gameCenter));

        post(BACKUP_MOVE, new PostBackupMove(gameCenter, gson));

        get(DARK_URL, new GetDarkRoute(gameCenter));
